...
a = Analysis([str(root / "packaging" / "launcher.py")], pathex=[str(root / "src")],
    ...
    noarchive=False)
pyz = PYZ(a.pure)
exe = EXE(pyz, a.scripts, [], exclude_binaries=True, name="RocketGNCMonitor",
...
collection = COLLECT(exe, a.binaries, a.datas, strip=False, upx=False, name="RocketGNCMonitor")
if sys.platform == "darwin":
    app = BUNDLE(collection, name="RocketGNCMonitor.app", icon=str(root / "build" / "icon.icns"), bundle_identifier="edu.mit.rocketteam.gncmonitor",
