...
## September 22 v0a workspace revision

v0a is maintained in `rocket_UI_v0a` alongside the unchanged v0 baseline. It adds two-window Base and reduced Away station workspaces, independent USB video/video-only recording, receive-only uncompressed APRS through external Dire Wolf, and explicitly connected read-only station snapshots over a local LAN. LTU equipment metrics, analog VRX tuning and audio/video calls remain unsupported pending their interfaces or implementation. See [station modes](V0A_STATION_MODES.md) and [board implementation notes](../references/GROUND_STATION_BOARD_NOTES.md). The dated results below are inherited v0 history, not v0a test claims.
