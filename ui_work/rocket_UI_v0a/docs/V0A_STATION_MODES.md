...
Mac is the current validation target. Windows Makefiles remain available for native builds; a successful Mac build does not establish Windows hardware or display behavior. The operator controls and original keyboard shortcuts should be checked with the connected equipment before field use.
