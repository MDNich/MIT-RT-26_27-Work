...
**v0a build note:** This folder builds version 0.1.1 as `RocketGNCMonitor-v0a.app` / `RocketGNCMonitor-v0a.exe`, with bundle ID `edu.mit.rocketteam.gncmonitor.v0a`. Apply the `-v0a` suffix to the inherited artifact names below. `make run` opens the startup wizard with remembered choices; `make away` and `make video` preselect those roles. `make video VEHICLE=iris` preselects Iris Video. Packaged verification checks the wizard, Base/Away telemetry, Balius/Iris receiver routing, Base local-Digital-only camera inventories and all Away thumbnail slots. The older dated validation results below describe v0 only; v0a has separate Apple Silicon, universal Mac, and Windows x64 packages. See [the v0a README](../README.md) for current commands and [the universal build guide](MAC_UNIVERSAL.md) for combining both native Mac payloads.
