...
**v0a build note:** This folder builds version 0.1.1 as `RocketGNCMonitor-v0a.app` / `RocketGNCMonitor-v0a.exe`, with bundle ID `edu.mit.rocketteam.gncmonitor.v0a`. Apply the `-v0a` suffix to the inherited artifact names below. `make run` opens Base station; `make away` opens Away station. `make verify-package` also runs `--station-smoke --station base` and `--station-smoke --station away`, checking the shared controller, window count and physical-transport isolation. The older dated validation results below describe v0 only; v0a has separate Apple Silicon, universal Mac, and Windows x64 packages. See [the v0a README](../README.md) for current commands and [the universal build guide](MAC_UNIVERSAL.md) for combining both native Mac payloads.
