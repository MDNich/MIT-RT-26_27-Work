...
**v0a workspace note:** Launch now opens the two-window Base station in LIVE; Away station is a reduced single-window layout. The page names below identify inherited controls, not navigation tabs in v0a. USB camera capture is independent of the telemetry board, and video-only logging is available once a stream has received frames. Rocket commands still require their own connected board. See [v0a station modes](V0A_STATION_MODES.md) for screen placement, APRS and the read-only LAN link.
