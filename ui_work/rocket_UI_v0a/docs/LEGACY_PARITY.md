...
**v0a placement note:** The inherited command and shortcut behavior below is retained, while the former eight pages are redistributed across the two Base station windows. Away station shows the reduced antenna/main-telemetry layout. USB video and video-only recording work without the ground-board serial connection. See [v0a station modes](V0A_STATION_MODES.md) for the current workspace.
