...
\src{src/rocket_gnc_monitor/widgets.py} & Themes, \code{AttitudeView}, \code{MountView}, animation and camera/pose transforms. \\
\src{src/rocket_gnc_monitor/mount_geometry.py} & Cached stand, turntable, beam, antennas and material definitions. \\
\src{src/rocket_gnc_monitor/scene_mesh.py} & Vectorized rigid transforms, normal shading and QPainter depth-sorted polygons. \\
\src{src/rocket_gnc_monitor/flight_scene.py} & Elapsed-time playback, quaternion SLERP, burn/recovery events and frame sampling. \\
\src{src/rocket_gnc_monitor/flight_view.py} & Modeless 3D viewer, orbit/pan/zoom, timeline, monitor-time link and overlays. \\
\src{src/rocket_gnc_monitor/rocket_geometry.py} & Cached schematic rocket, ignition plume and recovery canopy. \\
...
Optional motion columns are \code{qw,qx,qy,qz}, the three ENU velocity components
in metres/s, and roll/pitch/yaw rates in radians/s. They form a matching
$N\times10$ array; missing values are blank/NaN, never infinity. Partial column
sets are rejected. Metadata declares \code{visuals_schema_version=1},
\code{attitude_frame=body_to_ENU} and \code{body_axis=+Z}. A bounded
\code{flight_events} list stores time, type, source name and stable source ID.
The Java bridge exports actual simulation samples/events; trajectory save and
portable-flight snapshots preserve these additions without changing the base
four-column position contract.
...
The physical description is procedural code in
\src{src/rocket_gnc_monitor/mount_geometry.py}, specifically
\code{mount_mesh}. It is compiled once into a cached mesh, then drawn by
\code{MountView.paintEvent} in \src{src/rocket_gnc_monitor/widgets.py}.
...
\section{Smooth animation and 3D flight rendering}
\input{figures/flight-scene}
\code{Mesh} stores concatenated polygon vertices, group IDs, centers, normals
and cached material shades. Array operations apply fixed/azimuth/beam transforms,
project vertices and depth-sort faces; geometry is not rebuilt while painting.
\code{MountView} uses a precise 16 ms Qt timer, stops it while hidden, samples
virtual pose directly and approaches targets with a 45 ms exponential display
time constant. Azimuth uses the shortest signed arc through north. Fixed camera
framing removes pose-dependent zoom changes. Public command/pose state remains
separate from interpolated display state and never generates serial commands.

\code{FlightScene} and \code{Playback} follow the team's OpenRocket v6.2
\code{TrajectoryData}, \code{TrajectoryPlayback} and \code{Trajectory3DPanel}
conventions reviewed at commit \code{65defde5}. Playback uses a monotonic elapsed
clock; orientation uses normalized shortest-arc quaternion SLERP. Samples whose
angular rate times interval reaches $\pi$ radians do not invent intermediate
attitude. Missing orientation uses a labeled path-aligned illustration.
Ignitions and burnouts pair by motor source; old burnout-only events begin at
launch. Deployment enables the illustrative canopy; tumble/recovery marks
engine-held attitude. No recovery or ignition is guessed from position.

\code{Flight3DDialog} has its own visible-only timer, snapshots reference identity,
and resets when a reference changes. Monitor-time linking is read-only. Actual
track overlays require matching mission/reference origins. The trajectory drawing
uses at most 6,000 path vertices; full motion samples remain available for frame
sampling. Cached schematic vehicle/plume/canopy geometry uses the same mesh
renderer and Qt/NumPy runtimes as the antenna. No Python OpenGL dependency is added.

Local Cocoa measurements reached about 63 displayed frames/s for both views.
The isolated antenna median repaint fell from about 48 ms to 8 ms after caching;
full-window native drawing remained within the 16 ms target in the recorded
probe. These are development-Mac measurements, not a guaranteed frame rate on
other machines. The verification captures ignition, burnout and recovery from
an actual bundled-example simulation and separately checks the Zephyrus model.
...
\code{PointerLocation} uses the existing offline MGRS converter and keeps
canonical absolute coordinates across display-format changes. Relative entry
stores bearing $b$, horizontal distance $d$ and altitude difference $\Delta h$.
During mission validation, \code{pointer_from_launch} solves antenna latitude
and longitude on WGS84 with $h_a=h_l+\Delta h$ so that the launch point in the
antenna's ENU frame satisfies $E=d\sin b$ and $N=d\cos b$. A bounded Newton
iteration uses metre-scaled finite differences and a $10^{-5}$\,m residual
criterion. Inputs are limited to 100\,km; degenerate or nonconvergent coordinates
raise a visible validation error. No additional runtime dependency is needed.
Launch edits and disk loads re-resolve relative positions; absolute MGRS entry
retains its canonical latitude/longitude. Old mission files default to lat/lon.
Tests cover heading chirality, both hemispheres, high latitude, the antimeridian,
zero distance, invalid entries and mission/flight persistence.
...
\code{test_flight_3d.py} & SLERP, event boundaries, elapsed clock, motion persistence, animation and read-only Qt viewer controls. \\
...
\code{--flight-3d-smoke CSV} & Verify quaternion-backed 3D poses, ignition, burnout and parachute renderings. \\
...
The current graphics revision passed 113 Mac regression tests. The antenna-location revision passed 107; the preceding virtual-pointer revision passed 101. Native Cocoa popup checks cover both themes and long device names. The previous Settings/engine revision passed 94 tests. Settings persistence, shortcut handling, bundled/custom simulations and local/latest/pinned build paths are covered. The earlier dual-video revision passed 77 tests. New checks
