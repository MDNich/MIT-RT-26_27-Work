...
operations use a separate one-worker executor to serialize FFmpeg lifecycle
changes. A decoder thread drains FFmpeg frames, with a separate stderr drain.

The controller's 30 ms timer drains up to 2,000 queued messages per tick,
advances DEMO/REPLAY, dispatches tracking when due, takes the most recent video
frame, collects completed futures and updates alerts. Playback delta is capped
...
Video display queue & 2 frames & Old display frames are dropped for freshness, independently of FFmpeg recording. \\
...
Video uses a separate generation and a single lifecycle executor to prevent an
old open result from replacing a newer selection.
...
workers, closes video and waits for the video executor, cancels the active
...
\code{start_video} increments a token and schedules lifecycle work on the
single-worker video executor. \code{stop} first writes \code{q} and continues
...
and video directory. Replay finds the eligible recording segment at
\[
 t_{\mathrm{video}}=t_{\mathrm{replay}}-t_{\mathrm{start}}-\Delta_{\mathrm{offset}}.
\]
It chooses a segment whose interval contains that time and seeks relative to
the segment start. Positive offset therefore delays video. Paused replay uses
...
10 MB per motor curve, 120 s of worker wall time and 20 MB of worker log.
...
\begin{lstlisting}[language=bash]
make runtime
make bridge OPENROCKET_JAR="/absolute/path/to/swing-24.12.RC.01-all.jar"
\end{lstlisting}
...
\code{test_build.py} & Windows-on-ARM host with x64 Python selects the x64 runtime correctly. \\
\bottomrule
\end{longtable}
