...
It combines two independent serial links, camera video, recorded-flight playback,
...
LIVE & Physical Zephyrus telemetry; ground commands require its board connection. Pointer manual controls require the pointer board independently. & Logging captures received data. Source changes close physical transports and clear displayed history. Reconnect and restart polling explicitly. \\
...
\code{/dev/tty.} versus \code{/dev/cu.} alias.
...
\section{Video receiver and recorded video}
Choose \ui{Find cameras}, select a listed USB camera/receiver and press
\ui{Start camera}. The app uses AVFoundation on Mac, DirectShow on Windows and
...
ratio preserved. One FFmpeg input feeds both display and recording. Camera and
...
The video status reports frame age and errors; after two seconds without a
...
Starting or stopping logging reopens the configured video pipeline so its
recording destination changes. A camera reconnect during a recording uses a
...
A paused seek decodes a single frame. Camera exposure timestamps are not
...
Video is labeled \ui{VIDEO TEST PATTERN}; no actual test-launch video was supplied.
...
recording fault/loss and video ended/stale. Battery must remain below threshold
