...
\subsection{Orbitable 3D flight playback}
Open \ui{View > 3D flight}, or \ui{3D} beside the Flight overview trajectory
selector. The viewer shows a simulation reference, an enlarged schematic rocket
with fins/canards, a motor plume during recorded burns and a parachute at recorded
deployment. Body orientation follows exported quaternions independently of the
viewing camera. This is an illustrative vehicle, not an imported CAD surface.

Drag to orbit; Shift-drag or right-drag to pan; scroll to zoom. Double-click or
\ui{Fit} resets framing. Perspective, Top and Side presets, rocket size,
\ui{Follow rocket}, ground projection and velocity overlays are available.
Play/pause, rewind, speed, the timeline and event selector support inspection of
ignition, burnout, apogee, deployment and touchdown. Independent playback sends
no hardware command. \ui{Follow monitor time} instead follows virtual-pointer
rehearsal or aligned telemetry flight time; the displayed rocket remains the
simulation reference at that time.

Gold denotes the reference, teal the actual track when geographic origins match,
and purple the antenna location. Old four-column references remain usable with
a path-aligned illustration and explicit unavailable-attitude/events labels.
Rerun OpenRocket to add orientation and event data. Ignition and deployment are
never inferred from ascent/descent. The supplied Zephyrus test model has no
recovery event in its nominal output; the bundled small example does. Held
attitude after tumble/recovery and insufficient angular sampling are labeled.
Portable flight files retain the added columns and event metadata.
...
The antenna uses cached geometry and an independent 16 ms display timer. Rotation
is smoothed along the shortest azimuth arc; fixed framing avoids size changes
during movement. Hidden views stop animating. This affects presentation only,
without changing serial commands or controller timing.
...
\input{figures/antenna-hierarchy}
...
Configure the mount under \ui{Mission > Configure > Antenna pointer}. Select
latitude/longitude, \ui{MGRS}, or \ui{Relative to launch}, and check
\ui{Antenna location established}. Absolute entry uses a separate WGS84
ellipsoid altitude; MGRS accepts spaces and lowercase and previews grid precision.
Relative entry requires an established launch origin and takes the true heading
from the antenna toward the rocket on the launch pad, horizontal distance
(0--100\,km), and antenna altitude minus launch altitude. Positive height means
the antenna is above the pad. For example, $90^\circ$, 500\,m and $+5$\,m place
it west of the pad and 5\,m higher. The preview shows resolved coordinates.
Mission JSON and portable flight files retain the entry form and parameters;
relative positions recalculate when the mission launch origin changes. The physical-board
