...
\small
\textbf{Objet du document.} \it Ce rapport décrit d'abord le modèle thermique
transitoire actuellement exécuté, limité à la pyrolyse volumique de la couche
phénolique interne \code{phe0}. Il propose ensuite une extension qui reste dans
ANSYS Mechanical pour représenter le dégazage et une récession discrète par
suppression d'éléments. Fluent et la combustion du propergol ne font pas partie
du périmètre. Les valeurs marquées \emph{provisoires} doivent être remplacées ou
validées par des mesures du matériau effectivement fabriqué avant toute décision
...
\subsection{Dimensions Extraites du Modèle 3D}

Le fichier STEP AP214 utilise le mètre. Les surfaces cylindriques donnent les
rayons d'interface du tableau~\ref{tab:geometry}. Le modèle est un cylindre droit
de longueur \(L=\SI{1.8288}{m}\), soit exactement \SI{72}{in}; son alésage vaut
\SI{266.7}{mm} (\SI{10.5}{in}) et son diamètre extérieur \SI{304.8}{mm}
(\SI{12}{in}). Les épaisseurs totalisent \SI{19.05}{mm}.
...
\end{tikzpicture}
\caption{Empilement radial extrait du modèle 3D. La largeur des couches est
schématique; les cotes indiquées sont exactes.}
\label{fig:stack}
\end{figure}
...
\toprule
\textbf{Corps} & \textbf{Rayon intérieur} & \textbf{Rayon extérieur} & \textbf{Épaisseur} & \textbf{Masse initiale estimée} \\
 & (mm) & (mm) & (mm) & (kg) \\
\midrule
...
\bottomrule
\end{tabular}~\\[1em]
\caption{Géométrie du moteur. Les masses sont calculées avec les
densités initiales du modèle, sans fonds, joints, interfaces ni propergol.}
\label{tab:geometry}
\end{table}

L'aire cylindrique chaude est \(2\pi r_i L=\SI{1.5323}{m^2}\) et l'aire
extérieure \(2\pi r_o L=\SI{1.7512}{m^2}\). Ces valeurs seront utiles pour les
bilans d'énergie et de masse indépendants du solveur.
...
\subsection{Résultats de la simulation 1 (à compléter)}

\placeholder{Sous-section réservée aux résultats : températures, SVAR1,
SVAR2, profondeur de pyrolyse, bilans d'énergie et étude d'indépendance.}
...
\subsection{Implémentation dans Mechanical (à compléter)}

\placeholder{Sous-section réservée à l'implémentation future : architecture des
bandes, commandes MAPDL, transfert des charges, logique de redémarrage et
vérifications de conservation.}

\subsection{Résultats de la simulation 2 (à compléter)}

\placeholder{Sous-section réservée aux résultats : débit de dégazage, facteur de
soufflage, récession, chronologie des bandes supprimées et bilans masse-énergie.}
