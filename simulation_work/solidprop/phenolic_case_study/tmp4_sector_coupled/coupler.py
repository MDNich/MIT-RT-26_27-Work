...
def prepare_step(mesh,c,s):
    ...
    assert p['loaded_hot_faces']==200 and p['loaded_hot_nodes']==303
...
def accept_step(mesh,c,s,p,temperatures,fields,energy,time):
    ...
    s['status']='PHE0_CONSUMED' if s['layer']==64 else ('COMPLETED_HORIZON' if time>=c['end_time_s']-1e-10 else 'RUNNING')
    s['warnings']=warnings
    ...
    record=dict(time_s=time,dt_s=p['dt'],layer_solved=p['layer'],rows_removed=p['layer'],rows_scheduled=s['layer'],
                hot_radius_m=c['geometry']['inner_radius_m']+p['layer']*c['geometry']['phe0_thickness_m']/64,
                ...
                gas_cumulative_kg=s['gas_cumulative_kg'],char_cumulative_kg=s['char_cumulative_kg'],
...
def main():
    ...
    if args.action=='validate':
        assert len(mesh['rows'])==64 and all(len(r)==200 for r in mesh['rows'])
    ...
    if args.action=='prepare':
        stop=(root/'PAUSE_REQUESTED').exists() or s['time_s']>=c['end_time_s']-1e-10 or s['layer']>=64
        if stop:
        ...
        atomic(root/'simulation4_history.csv',buf.getvalue())
